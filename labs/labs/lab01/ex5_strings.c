...
int main() {
  ...
  // printf("prints world: %s\n", *world_str); --> Don't add "*".            
  /**
   * 1. world_str evaluates to the character 'w' (ASCII value 119).
   * 2. printf with %s expects a pointer (a memory address) to a null-terminated string.
   * 3. printf will treat 119 as a memory address and try to read a string starting at 0x77.
   * 4. That’s an invalid address → segmentation fault.
   */
